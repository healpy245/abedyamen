...
final class CategoryIngredientsStoreWorkflow extends AbstractFormWorkflow
{
    public function run(array $payload, ?callable $onProgress = null): array
    {
        ...
        try {
            ...
            $progress('categories', 'Creating ingredients categories via Kaman API...', []);
            $createResult = $this->createIngredientsCategories($baseUrl, $token, $categories, $progress);
            $progress('categories', 'Created ' . count($createResult['created']) . ' categories, ' . count($createResult['failed']) . ' failed', $createResult);

            Log::info('CategoryIngredientsStoreWorkflow completed', [
            ...
            return [
                'success' => true,
                'message' => 'Category ingredients store processed successfully',
                'data' => [
                    'token' => $token,
            ...
            Log::error('CategoryIngredientsStoreWorkflow failed', [
                'error' => $e->getMessage(),
                'restaurant' => $restaurantName,
            ]);
    ...
    private function login(string $baseUrl, string $subdomain, string $password): string
    {
        ...
        if (!$response->successful()) {
            ...
            Log::warning('CategoryIngredientsStoreWorkflow login failed', [
    ...
    {
        $systemPrompt = <<<PROMPT
You are a restaurant ingredients category parser. You receive a list of category names (in any language or format).
...
PROMPT;
        ...
        $userPrompt = "Ingredients categories to parse:\n{$description}";
    ...
    /**
     * Create each ingredients category via the Kaman API.
     *
     ...
     */
    private function createIngredientsCategories(string $baseUrl, string $token, array $categories, ?callable $progress = null): array
    {
        $created = [];
        $failed = [];
        ...
        foreach ($categories as $key => $category) {
            $i++;
            $progress && $progress('category', 'Creating ingredients category ' . $i . '/' . $total . ': ' . ($category['name_en'] ?? $key), ['key' => $key]);

            $response = $this->http()
                ->withToken($token)
                ->post("{$baseUrl}/api/manager/ingredients-categories", $category);
            ...
            if ($response->successful()) {
                ...
                Log::warning('CategoryIngredientsStoreWorkflow category creation failed', [
                    'key' => $key,
                    'status' => $response->status(),
                    'response' => $message,
                ]);
